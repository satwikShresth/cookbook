	...
	useEffect(() => {
		...
		const nodes: NodeDatum[] = allFiles
			...
			.map((f) => ({
				...
				numLinks: nav.edges.filter(
					(e) => e.source === f.slug || e.target === f.slug,
				).length,
			}));
		...
		const colorFg =
			style.getPropertyValue("--chakra-colors-fg").trim() || "#111";
		const colorMuted =
			style.getPropertyValue("--chakra-colors-fg-muted").trim() || "#888";
		const colorBorder =
			style.getPropertyValue("--chakra-colors-border").trim() || "#ddd";
		const colorAccent =
			style.getPropertyValue("--chakra-colors-blue-500").trim() || "#3b82f6";
		...
		const nodeRadius = (d: NodeDatum) =>
			d.isCurrent ? 6 : 2 + Math.sqrt(d.numLinks + 1);
		...
		const sim = d3
			...
			.force(
				"collide",
				d3.forceCollide<NodeDatum>((d) => nodeRadius(d) + 8),
			);
